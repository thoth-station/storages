    ...
    for document_id in document_ids or adviser_store.get_document_listing():
        ...
        if force or not graph.adviser_document_id_exist(os.path.basename(document_id)):
            try:
                if is_local:
                    _LOGGER.debug(
                        "Loading document from a local file: %r", document_id)
                ...
                _LOGGER.exception(
                    "Failed to sync adviser result with document id %r", document_id)
                failed += 1
        else:
            _LOGGER.info(
                f"Sync of adviser document with id {document_id!r} skipped - already synced")
            skipped += 1
        ...
        if force or not graph.build_log_analysis_document_id_exist(os.path.basename(document_id)):
            try:
                if is_local:
                    ...
                    document = build_log_analysis_store.retrieve_document(
                        document_id)
                ...
                _LOGGER.exception(
                    "Failed to sync build log analysis result with document id %r", document_id)
                failed += 1
        else:
            _LOGGER.info(
                "Sync of build log analysis document with id %r skipped - already synced", document_id)
            skipped += 1
        ...
        if force or not graph.package_analysis_document_id_exist(os.path.basename(document_id)):
            try:
                if is_local:
                    ...
                    document = package_analysis_store.retrieve_document(
                        document_id)
                ...
                _LOGGER.exception(
                    "Failed to sync package analysis result with document id %r", document_id)
                failed += 1
        else:
            _LOGGER.info(
                f"Sync of package analysis document with id {document_id!r} skipped - already synced")
            skipped += 1
        ...
        if force or not graph.provenance_checker_document_id_exist(os.path.basename(document_id)):
            try:
                if is_local:
                    ...
                    document = provenance_check_store.retrieve_document(
                        document_id)
                ...
                _LOGGER.exception(
                    "Failed to sync provenance-checker result with document id %r", document_id)
                failed += 1
        else:
            _LOGGER.info(
                f"Sync of provenance-checker document with id {document_id!r} skipped - already synced")
            skipped += 1
        ...
        if force or not graph.dependency_monkey_document_id_exist(os.path.basename(document_id)):
            try:
                if is_local:
                    ...
                    document = dependency_monkey_reports_store.retrieve_document(
                        document_id)
                ...
                _LOGGER.exception(
                    "Failed to sync dependency-monkey result with document id %r", document_id)
                failed += 1
        else:
            _LOGGER.info(
                f"Sync of dependency-monkey document with id {document_id!r} skipped - already synced")
            skipped += 1
    ...
    *,
    ...
    for inspection_id in document_ids or dependency_monkey_reports_store.iterate_inspection_ids():
        ...
        if force or not inspection_store.document_exists(inspection_id):
            ...
            if finished:
                _LOGGER.info(
                    "Obtaining results from Amun API for inspection %r", inspection_id)
                try:
                    ...
                    if has_inspection_job(amun_api_url, inspection_id):
                        # TODO: load and sync job logs separately from workflow artifacts to decrease memory usage
                        job_logs = get_inspection_job_logs(
                            amun_api_url, inspection_id)
                    ...
                    document = {
                        ...
                        "created": created,
                        "build_log": build_log,
                        # TODO: take job_logs out of this document
                        "job_logs": job_logs,
                    ...
                    try:
                        _ = INSPECTION_SCHEMA(document)
                    except (Invalid, MultipleInvalid) as exc:
                        _LOGGER.error(
                            f"Failed to sync inspection {inspection_id}: "
                            f"Invalid inspection document schema: {exc}"
                        )
                        failed += 1
                        continue
                    ...
                    _LOGGER.exception(
                        f"Failed to sync inspection %r: %s", inspection_id, str(exc))
                    failed += 1
            else:
                _LOGGER.info(
                    f"Skipping inspection {inspection_id!r} - not finished yet")
                skipped += 1
        else:
            _LOGGER.info(
...
def sync_documents(
    ...
    *,
    ...
    if inspection_only_ceph_sync and inspection_only_graph_sync:
        raise ValueError(
            "Parameters `inspection_only_ceph_sync' and `inspection_only_graph_sync' are disjoint")
    ...
    for document_id in document_ids or [None] * len(_HANDLERS_MAPPING):
        for document_prefix, handler in _HANDLERS_MAPPING.items():
            ...
            if document_id is None or os.path.basename(document_id).startswith(document_prefix):
                ...
                stats[document_prefix] = tuple(
                    map(sum, zip(stats[document_prefix], stats_change)))
