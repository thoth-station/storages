...
class PerformanceIndicatorBase:
    ...
    def create_from_report(
        cls, session: Session, specification: dict, inspection_log: dict, inspection_run_id: int
        ...
        overall_score = inspection_log["stdout"].get("overall_score")
        ...
        partial_model = partial(
            ...
            component=inspection_log["stdout"].get("component"),
            origin=specification.get("script"),
            version=inspection_log["stdout"].get("version")
            or inspection_log["script_sha256"],
            overall_score=overall_score,
            exit_code=inspection_log.get("exit_code"),
            ru_utime=inspection_log.get("usage", {}).get("ru_utime"),
            ru_stime=inspection_log.get("usage", {}).get("ru_stime"),
            ru_maxrss=inspection_log.get("usage", {}).get("ru_maxrss"),
            ru_ixrss=inspection_log.get("usage", {}).get("ru_ixrss"),
            ru_idrss=inspection_log.get("usage", {}).get("ru_idrss"),
            ru_isrss=inspection_log.get("usage", {}).get("ru_isrss"),
            ru_minflt=inspection_log.get("usage", {}).get("ru_minflt"),
            ru_majflt=inspection_log.get("usage", {}).get("ru_majflt"),
            ru_nswap=inspection_log.get("usage", {}).get("ru_nswap"),
            ru_inblock=inspection_log.get("usage", {}).get("ru_inblock"),
            ru_oublock=inspection_log.get("usage", {}).get("ru_oublock"),
            ru_msgsnd=inspection_log.get("usage", {}).get("ru_msgsnd"),
            ru_msgrcv=inspection_log.get("usage", {}).get("ru_msgrcv"),
            ru_nsignals=inspection_log.get("usage", {}).get("ru_nsignals"),
            ru_nvcsw=inspection_log.get("usage", {}).get("ru_nvcsw"),
            ru_nivcsw=inspection_log.get("usage", {}).get("ru_nivcsw"),
        )

        return cls.from_report(inspection_log, partial_model)

    @classmethod
    def from_report(cls, inspection_log: dict, partial_model: type(partial)) -> "PerformanceIndicatorBase":
        """Create model from the inspection report respecting parameters and result reported by the indicator."""
        kwargs = {}
        for parameter, parameter_value in inspection_log["stdout"]["@parameters"].items():
            kwargs[parameter] = parameter_value

        for result_name, result_value in inspection_log["stdout"]["@result"].items():
